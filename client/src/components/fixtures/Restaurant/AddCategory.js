...
class AddCategory extends Component {
    ...
    if (imageValue === "") {
      ...
      this.setState({errors});
        ...
        await axios.post(`${getHost()}/Category/insert`,formData).then(resp=>{
            this.setState({
    ...
    render() {
        return (
            ...
            <form onSubmit={this.handleSubmit}>
                    ...
                    <div className="form-group">
                        <img style={{ width: '100%' }} src={this.state.filepath} alt=''/>
                    </div>
                    <p className="result-error-message">{this.state.Errormessage}</p>
                    <p className="result-success-message">{this.state.Successmessage}</p><br></br>

                    <input type="submit" value="Add Category" className="form-control submit-add"/>
            </form>
            </div>
            </div>
        </div>
