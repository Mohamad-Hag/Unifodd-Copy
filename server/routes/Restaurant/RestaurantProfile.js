...
router.put('/edit',(req,res)=>{
    ...
    const update = "UPDATE restaurant SET Password =? WHERE Password = ? and ID =? "
    db.query(update,[newPassword,oldPassword,restID],(err)=>{
        if(err){ 
            console.log(err);
            return res.json({
                PasswordErrormessage: "Something went wrong, please fill all the records"
            }); 
        }
        else{
        return res.json({
...
router.put('/editGeneral',(req,res)=>{
    ...
    db.query(update,[username,phone],(err)=>{
        if(err){ 
            console.log(err);
            return res.json({
                GeneralErrormessage: "Something went wrong, please fill all the records"
            }); 
        }
        else{
        return res.json({
...
router.put('/editImage',(req,res)=>{
    ...
    db.query(update,[image.name,restID],(err,result)=>{
      ...
      image.mv(`public/images/restaurants/${restaurantName}/profile-image/${image.name}`, err => {
        if (err) {
           console.error(err);
         }
        })
