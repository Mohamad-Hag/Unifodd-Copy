...
class AdminDashboard extends Component {
  constructor(props) {
    ...
    // Bindings Methods
    this.notificationMarkAsReadClicked = this.notificationMarkAsReadClicked.bind(
      this
    );
    this.notificationsButtonClicked = this.notificationsButtonClicked.bind(
      this
    );
  ...
  getNotifications() {        
    ...
    let pro = new Promise( async (resolve) => {
      ...
      await axios.post(`${getHost()}/api/admin/notification/`, formData).then( res => {
        let data = res.data;   
        console.log(data);     
        for (let el of data)  {
          notifications.push({
  ...
  componentDidMount = () => {    
    if(!Cookies.get("id")){
      ...
      const formData = new FormData();
      formData.append('userID', Cookies.get("id"));
      axios.post(`${getHost()}/api/admin/notification/count`, formData).then( res => {
        this.setState({
  ...
  render() {
    return (
      ...
      <div>
        ...
        <div className="admin-flex admin-vh100 w100 admin-border-box admin-display-none" id="Dashboard">
          <AdminSidebar />
          <div className="admin-main-content admin-overflow-auto admin-relative admin-background-f2 admin-border-box h100">
            <Switch>
              <Route exact path="/admin"><Statistics /></Route>
              <Route exact path="/admin/restaurants"><Restaurant /></Route>
              <Route exact path="/admin/users"><User /></Route>
              <Route exact path="/admin/blacklist"><BlackList /></Route>
              <Route exact path="/admin/hidden"><Hidden /></Route>
              <Route exact path="/admin/add"><AddRestaurant /></Route>
              <Route exact path="/admin/restaurants/:name" render={(props) => <Details {...props} />} />
              <Route exact path="/admin/users/:id" render={(props) => <UserLog {...props} />} />
              <Route exact path="/admin/notifications"><Notifications /></Route>
            </Switch>
          </div>
        </div>
        <Header
