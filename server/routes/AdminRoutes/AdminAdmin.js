...
router.get('/stat/rest', (req, res) => {
    let getRests = "SELECT u.Name FROM restaurant r JOIN `user` u ON u.ID = r.UserID";
    let data = [];
    let i = 1;
    db.query(getRests, (err, result) => {
        if(err) console.log(err)
            ...
            result.map((item) => {
                let query = 'SELECT COUNT(o.ID) as counter FROM `order` o JOIN order_item oi ON o.ID = oi.OrderID JOIN product p ON p.ID = oi.ProductID JOIN category ca ON ca.ID = p.CategoryID JOIN restaurant r ON r.ID = ca.RestaurantID JOIN `user` u ON u.ID = r.UserID WHERE u.Name = "'+item.Name+'"';
                db.query(query, (err, result2) => {
                    ...
                    if(i == result.length)
                        db.query('SELECT COUNT(ID) as counter FROM `order`', (err, result) => {
                            if(err) console.log(err)
                            else{
                                return res.json({
