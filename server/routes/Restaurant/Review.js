...
router.get('/get',(req,res)=>{
  ...
  const sqlSelect = `SELECT r.Text, r.Date, r.Like, r.Dislike, r.CustomerID, r.ProductID, p.ID AS ProductID, p.Name, c.ID, c.UserID, u.ID, u.Name AS UserName FROM review r, product p, customer c, user u, category ca WHERE p.CategoryID = ca.ID and ca.RestaurantID =${restID} and r.CustomerID = c.ID and r.ProductID = p.ID and c.UserID = u.ID GROUP BY r.ID;`
